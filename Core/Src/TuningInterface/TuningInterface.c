...
void tunePID(UART_HandleTypeDef* huartx, PSxBT_t* ps4, RNS_interface_t* rns){
	...
	if(ps4->button == UP){
		...
		while(ps4->button == UP){
			...
			{
				VESCNavVel(target_speed, target_speed, target_speed, target_speed);
				sprintf(data, "%.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f\n",
						target_speed, VESCNav.a_vel, target_speed, VESCNav.b_vel, target_speed, VESCNav.a_vel, target_speed, VESCNav.a_vel);
				if(HAL_GetTick()-tick >= 20){
					tick = HAL_GetTick();
					UARTPrintString(huartx, data);
				}
		...
		while(ps4->button == LEFT){
			...
			else
			{
				VESCNavVel(-target_speed, target_speed, target_speed, -target_speed);
				sprintf(data, "%.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f\n",
						target_speed, -VESCNav.a_vel, target_speed, VESCNav.b_vel, target_speed, VESCNav.c_vel, target_speed, -VESCNav.d_vel);
				if(HAL_GetTick()-tick >= 20){
					tick = HAL_GetTick();
					UARTPrintString(huartx, data);
				}
